...
- **Soft `/ready` deps** — LangSmith / OpenRouter reachability fields remain soft; hard `ready` still = gateway + OpenRouter **key**. HTTP status is **503** when `ready:false` (orchestrator-friendly); body still includes `ready` boolean (`test_health.py`).
...
| `/ready` HTTP | **200** when `ready:true`; **503** when `ready:false` (JSON body unchanged; chat gate still reads boolean) |
