...
/**
 * Internal verify-disclosure callback for the Co-Pilot sidecar (no browser CSRF).
 *
 * Auth: X-Copilot-Internal-Secret. Bind required via correlation store.
 *
 * @package   OpenEMR
...
try {
    ...
    $service = new VerifyDisclosureService($bindStore, $secret, $disclosureLog);
    $result = $service->handle($body, $providedSecret, $correlationId);
} catch (Throwable $e) {
    error_log('Ask Co-Pilot disclosure error: ' . $e->getMessage());
    http_response_code(500);
